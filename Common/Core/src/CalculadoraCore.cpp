...
namespace Finexa {
    ...
    std::shared_ptr<Moneda> CalculadoraCore::buscarMoneda(const std::string& siglas) {
    ...
    std::shared_ptr<TasaDeCambio> CalculadoraCore::buscarTasa(const std::string& base, const std::string& destino) {
        auto it = std::find_if(tasas.begin(), tasas.end(), [&](const auto& t) {
            return t->getBase()->getSiglas() == base && t->getDestino()->getSiglas() == destino;
        });
        return (it != tasas.end()) ? *it : nullptr;
    }

    void CalculadoraCore::establecerTasa(const std::string& siglasBase, const std::string& siglasDestino, double valor) {
    ...
    double CalculadoraCore::calcularValorImplicito(const std::string& base, const std::string& destino) {
        if (base == destino) return 1.0;
        
        // Triangulacion: (VES/Destino) / (VES/Base)
        double tasaVesBase = 0.0;
        double tasaVesDestino = 0.0;

        if (base == PIVOTE) tasaVesBase = 1.0;
        else {
            auto t = buscarTasa(PIVOTE, base);
            if (t) tasaVesBase = t->getValor();
        }

        if (destino == PIVOTE) tasaVesDestino = 1.0;
        else {
            auto t = buscarTasa(PIVOTE, destino);
            if (t) tasaVesDestino = t->getValor();
        }

        if (tasaVesBase == 0.0) return 0.0;
        return tasaVesDestino / tasaVesBase;
    }

    double CalculadoraCore::convertirAXds(double monto, const std::string& siglasOrigen) {
        if (siglasOrigen == OBJETIVO) return monto;
        ...
        if (!tasaVesXds || tasaVesXds->getValor() == 0.0) return 0.0;

        double montoVes = monto * tasaVesOrigen;
        return montoVes / tasaVesXds->getValor();
    }

    bool CalculadoraCore::tieneAmbiguedad(const std::string& base, const std::string& destino, double valorManual) {
        double valorCalculado = calcularValorImplicito(base, destino);
        if (valorCalculado == 0.0) return false;
    ...
    const std::vector<std::shared_ptr<Moneda>>& CalculadoraCore::getMonedas() const { return monedas; }
    const std::vector<std::shared_ptr<TasaDeCambio>>& CalculadoraCore::getTasas() const { return tasas; }
