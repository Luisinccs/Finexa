...
MonedasViewModel::MonedasViewModel(
  ...
  _dialogViewModel = DcDialogViewModel::crearDialogViewModel();
...
IMPLEMENT_CONTROL_INTERNAL(Finexa::ViewModels, MONEDAS_VIEW_MODEL, DIALOG,
                           Dialog)
  ...
  if (vmPtr) {
    auto vm =
        *static_cast<std::shared_ptr<Finexa::ViewModels::MonedasViewModel> *>(
            vmPtr);
    if (vm) {
      // Use the public binding accessor generated by the macro
      return vm->dialogViewModelBinding();
    }
  }
  return nullptr;
}
